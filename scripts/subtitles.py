...
'''
    for x in out:
        text='\\N'.join(x['lines']);tag='{\\q2\\fs'+str(x['font_size'])+'}'
    ...
    report=dict(version=VERSION,status='prepared_needs_visual_and_audio_review',max_lines=max(len(x['lines']) for x in out),cues=len(out),warnings=warnings,not_checked=['semantic_accuracy','full_audio_sync','silence_hallucinations_without_VAD','important_UI_occlusion','visual_render_bounds'],video_sha256=m['video_sha256'],captions_sha256=sha(revision/'captions.json'),font_sha256=sha(revision/'fonts'/('font'+fontpath.suffix)))
...
def main():
    ...
    for name in ['transcribe','prepare','preview','render','import-srt']:
    ...
    a=p.parse_args()
    try:{'doctor':doctor,'init':init,'transcribe':transcribe,'import-srt':importsrt,'prepare':prepare,'preview':render,'render':render}[a.command](a)
