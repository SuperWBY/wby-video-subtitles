...
```text
現在使用している Agent に WBY Video Subtitles をインストールしてください。
https://github.com/SuperWBY/wby-video-subtitles

まず README と SKILL.md を読み、現在のクライアントの設定やドキュメントから実際の Skill ディレクトリを確認して、リポジトリ全体をインストールしてください。Codex を使用していると決めつけないでください。既存のインストールがある場合は、取得元とローカル変更を確認し、変更を保持したうえで更新方法を判断してください。

独立した Python 環境に必要な依存関係をインストールし、FFmpeg/libass を確認して、OS に適した音声認識バックエンドを選んでください。同梱フォントと既定のスタイルを使い、scripts/subtitles.py doctor を実行してください。最後に、インストール先、依存関係の状態、クライアントが Skill を検出できるかを報告してください。再読み込みや新しいセッションが必要なら、具体的な次の操作を説明してください。ローカル実行やインストール先の確認ができない場合は、不足しているものを説明し、成功したと報告しないでください。
```
